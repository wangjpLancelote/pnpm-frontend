...
watch(newexpression, () => resolveExp())
function shouldHide(key) {
  return !(newhideComponent.value && newhideComponent.value.includes(key))
}
function resolveExp() {
  // 反解析 表达式
  if (newexpression.value) {
    const arr = newexpression.value.split(/\s+/)
    if (arr.length >= 6) {
      //6 位以上是合法表达式
      let obj = {
        second: arr[0],
...
onMounted(() => {
  newexpression.value = props.expression
  newhideComponent.value = props.hideComponent
})
</script>
