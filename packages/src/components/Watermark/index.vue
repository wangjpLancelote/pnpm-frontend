<template>
  <div class="watermark" ref="watermark" :style="{ maxHeight: typeof height === 'number' ? (height + 'px') : height }" @click="handleDefault">
    <div class="watermark-field" v-for="(item, index) in fillAmount" :key="index">{{ text }}</div>
  </div>
</template>
...
const fillAmount = computed(() => {
  const maxHeight = typeof props.height == 'number' ? props.height : parseInt(props.height);
  const maxWidth = typeof props.width == 'number' ? props.width : parseInt(props.width);
